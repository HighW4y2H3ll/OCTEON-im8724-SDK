...
/** 802.1q mode for a port */
enum qca833x_17_vlan_mode {
	/** 802.1q is disabled, use port-based VLAN only */
	MODE_8021q_DISABLE = 0,
	/**
	 * Enable 802.1q for all frames.  Do not discard ingress membership
	 * violations and use the port-based VLAN if the frame's VID is not
	 * contained in the VLAN table.
	 */
	MODE_8021q_FALLBACK = 1,
	/**
	 * Enable 802.1q for all received frames.  Do not discard ingress
	 * membership violations but discard frames where the VID is not
	 * contained in the VLAN table.
	 */
	MODE_8021q_CHECK = 2,
	/**
	 * Enable 802.1q for all received frames.  Discard frames with
	 * ingress membership violations or whose VID is not contained in the
	 * VLAN table.
	 */
	MODE_8021q_SECURE = 3
};

enum qca833x_17_port_state {
	/** Port is completely disabled and cannot receive or transmit frames */
	PORT_STATE_DISABLED = 0,
	/**
	 * Only management frames are forwarded to the designated port, all
	 * other frames are dropped.
	 */
	PORT_STATE_BLOCKING = 1,
	/**
	 * Only management frames are transmitted and received.  All other
	 * frames are discarded.
	 */
	PORT_STATE_LISTENING = 2,
	/**
	 * Only management frames are transmitted and received.  All other
	 * frames are used for source address learning.
	 */
	PORT_STATE_LEARNING = 3,
	/**
	 * The port learns all source addresses and transmits and receives
	 * all frames like normal.
	 */
	PORT_STATE_FORWARDING = 4
};

enum qca833x_17_port_vlan_egress_mode {
	/** Transmit frames unmodified */
	EGRESS_TX_UNMODIFIED = 0,
	/** Transmit frames without a VLAN */
	EGRESS_TX_NO_VLAN = 1,
	/** Transmit frames with a VLAN */
	EGRESS_TX_VLAN = 2,
	/** Untouched (default) */
	EGRESS_TX_UNTOUCHED = 3
};

enum qca833x_17_port_vlan_ingress_mode {
	/** All frames can be received (default) */
	INGRESS_ALL_FRAMES = 0,
	/** Only 802.1q tagged frames can be received */
	INGRESS_TAGGED_FRAMES = 1,
	/** Only untagged frames can be received */
	INGRESS_UNTAGGED_FRAMES = 2
};

struct qca833x_17_port_vlan_config {
	/** VLAN mode for port */
	enum qca833x_17_vlan_mode		vlan_mode;
	/** Port state */
	enum qca833x_17_port_state		port_state;
	/** Port egress VLAN mode */
	enum qca833x_17_port_vlan_egress_mode	egress_mode;
	/** Port ingress VLAN mode */
	enum qca833x_17_port_vlan_ingress_mode	ingress_mode;
	/** Set to 1 to configure port, 0 to skip it */
	uint32_t configure:1;
	/** Enable source address learning */
	uint32_t learn_enable:1;
	/** Loopback all ports received on port */
	uint32_t ext_loopback:1;
	/** Leak ARP packets across VLANs */
	uint32_t leak_arp:1;
	/** Leak multicast frames across VLANs */
	uint32_t leak_multicast:1;
	/** Leak unicast frames across VLANs */
	uint32_t leak_unicast:1;
	/** Drop all received multicast frames */
	uint32_t drop_multicast:1;
	/**
	 * Force port-based VLAN enable.  If set use port-based VLAN and VLAN
	 * table to determine destination port.
	 */
	uint32_t force_port_vlan:1;
	/** Customer (inner) VLAN priority */
	uint32_t port_cvlan_priority:3;
	/** Service (outer) VLAN priority */
	uint32_t port_svlan_priority:3;
	/** Customer (inner) VLAN ID */
	uint32_t port_default_cvid:12;
	/** Service (outer) VLAN ID */
	uint32_t port_default_svid:12;
	/** Only send tagged frames */
	uint32_t eg_tagged_only:1;
	/**
	 * A 1 bit for each port indicates that the port is in the port-based
	 * vlan.  A port should not belong to its own VLAN.
	 */
	uint32_t port_vlan_map:7;
};

extern u32 qca8334_17_reg_read(struct phy_device *phydev, u32 reg_addr);
extern void qca8334_17_reg_write(struct phy_device *phydev, u32 reg_addr,
				 u32 reg_val);
