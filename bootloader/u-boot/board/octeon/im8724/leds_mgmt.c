...
void pim_leds_mgmt(int slot)
{
	...
	debug("Start pim%d leds processing...\n", slot);
	...
	for (port = 0; port < GET_MAX_SLOT_PORTS(slot); port++) {
		...
		if (sfp_pres == 0) {
			...
			if((sfp[slot][port].sfp_pres != sfp_pres)
				&& (IS_CPLD_IM13233(slot)))
			{
				/* Set max SFP+ rate, if this is configured */
				set_sfp_rate(slot, port, speed, 0, 0);
			}
			...
			if (sfp_rxlos == 1) {
				...
				if((speed != get_sfp_rate(slot, port, 0)) && !sfp_mm_allow) {
					speed = SPEED_UNDEF;
		...
		if(sfp_pres) {
			debug("  port: %d:%d, speed: %s, activity: %s\n",
				slot, port, speed_1g ? "1G" : "10G", activity ? "on" : "off");
		} else {
			debug("  port: %d:%d: no SFP\n", slot, port);
		}
	}
	debug("[done]\n");
}
...
void fp_leds_mgmt(void)
{
	...
	debug("Start fp leds processing...\n");
	if (first_time) {
	...
	for (port = 0; port < fp_ports_num; port++) {
		...
		if (sfp_pres == 0) {
			...
			if (sfp_rxlos == 1) {
				...
				if((speed != get_sfp_rate(-1, port, 0)) && !sfp_mm_allow) {
					speed = SPEED_UNDEF;
					activity = ACTIVITY_UNDEF;
				} else {
					speed_1g = (speed == SPEED_1G);
		...
		debug("  port: %d, speed: %s, activity: %s\n",
...
void sys_leds_mgmt(void)
{
	...
	debug("Start sys leds processing... ");
...
#define PS_NOT_PRSNT 0x00
	...
	/* Set st_led blinking red if overtemp */
	val = (cpld_rd(CPLD_ST_LED_CTL) & 0xF0);
	if ((cpld_rd(CPLD_ADDR_FLT_STAT) & CPLD_TM_OVERT)) {
		val |= LED_CLR_RED << 2;
		val |= LED_CNTRL_BLINKING;
		cpld_wr(CPLD_ST_LED_CTL, val);
	}
...
void leds_mgmt(void)
{
	...
	if(sfp_mm_allow == (-1))
		sfp_mm_allow = getenv("sfp_mm_allow") ? 1 : 0;
...
static uint8_t read_pim_qsfp_rxlos(int pim, int slot)
{
	...
	return !!(val & (QSFP_RX1_LOS | QSFP_RX2_LOS | QSFP_RX3_LOS | QSFP_RX4_LOS));
}
