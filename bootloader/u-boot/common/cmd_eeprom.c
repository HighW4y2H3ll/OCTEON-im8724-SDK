...
extern int  eeprom_write (unsigned dev_addr, unsigned offset,
			  uchar *buffer, unsigned cnt);
#ifdef CONFIG_CMD_SPI_EEPROM
extern int spi_eeprom_read(unsigned dev_addr, unsigned offset,
			   uchar *buffer, unsigned cnt);
extern int spi_eeprom_write(unsigned dev_addr, unsigned offset,
			    uchar *buffer, unsigned cnt);
#endif
...
/*-----------------------------------------------------------------------
 */
int do_spi_eeprom(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	const char *const fmt =
		"\nEEPROM %s: addr %08lx  off %04lx  count %ld ... ";
	...
	if (argc == 5) {
		...
		if (strcmp (argv[1], "read") == 0) {
			...
			printf (fmt, argv[1], addr, off, cnt);

			rcode = spi_eeprom_read(dev_addr, off,
						(uchar *) addr, cnt);
			...
			rcode = spi_eeprom_write(dev_addr, off,
						 (uchar *) addr, cnt);
...
int spi_eeprom_read(unsigned dev_addr, unsigned offset, uchar *buffer,
		    unsigned cnt)
{
	unsigned end = offset + cnt;
	unsigned blk_off;
...
int spi_eeprom_write(unsigned dev_addr, unsigned offset, uchar *buffer,
		     unsigned cnt)
{
	unsigned end = offset + cnt;
	unsigned blk_off;
	...
	debug("%s(%d, 0x%x, %p, %d)\n", __func__, dev_addr, offset, buffer, cnt);
#if defined(CONFIG_SYS_EEPROM_WREN)
	eeprom_write_enable(dev_addr,1);
#endif
	/* Write data until done or would cross a write page boundary.
...
#if defined(CONFIG_SYS_SPI_EEPROM_PAGE_WRITE_BITS)

#define	SPI_EEPROM_PAGE_SIZE	(1 << CONFIG_SYS_SPI_EEPROM_PAGE_WRITE_BITS)
#define	SPI_EEPROM_PAGE_OFFSET(x)	((x) & (SPI_EEPROM_PAGE_SIZE - 1))

		maxlen = SPI_EEPROM_PAGE_SIZE - SPI_EEPROM_PAGE_OFFSET(blk_off);
#else
		maxlen = 1;
#endif
		if (len > maxlen)
...
#if defined(CONFIG_SYS_SPI_EEPROM_PAGE_WRITE_DELAY_MS)
		udelay(CONFIG_SYS_SPI_EEPROM_PAGE_WRITE_DELAY_MS * 1000);
#endif
	}
#if defined(CONFIG_SYS_EEPROM_WREN)
	eeprom_write_enable(dev_addr,0);
...
U_BOOT_CMD(
	spieeprom,	5,	1,	do_spi_eeprom,
	"SPI EEPROM sub-system",
	"read  addr off cnt\n"
	"spi-eeprom write addr off cnt\n"
	"        - read/write `cnt' bytes at EEPROM offset `off'"
);
#endif
