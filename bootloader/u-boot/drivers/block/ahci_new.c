...
static int ahci_fill_sg(u8 port, unsigned char *buf, int buf_len)
{
	...
	for (i = 0; i < sg_count; i++) {
		sg_addr = (dma_addr_t)pci_virt_to_mem(probe_ent->dev,
						      (u32)buf + i * MAX_DATA_BYTE_COUNT);
		ahci_sg->addr =
		    cpu_to_le32((u32) sg_addr);
		ahci_sg->addr_hi = cpu_to_le32((u32)((sg_addr >> 16) >> 16));
...
{
	dma_addr_t cmd_addr = (dma_addr_t)pci_virt_to_mem(probe_ent->dev,
							  pp->cmd_tbl & 0xffffffff);
	pp->cmd_slot->opts = cpu_to_le32(opts);
	pp->cmd_slot->status = 0;
	pp->cmd_slot->tbl_addr = cpu_to_le32((u32)cmd_addr);
	pp->cmd_slot->tbl_addr_hi = cpu_to_le32((cmd_addr >> 16) >> 16);
}
 ...
 */
static int sata_dev_read_write(int dev_no, lbaint_t blknr, lbaint_t blkcnt,
			       int is_write, ulong * buffer)
{
	u64 lba = 0;
	u32 len = 0;
	...
	fis.pm_port_c = (1 << 7);
	if (is_write) {
		fis.command = ATA_CMD_WRITE;
		fis.prot = cpu_to_le16(PRB_PROT_WRITE);
	} else {
		fis.command = ATA_CMD_READ;
		fis.prot = cpu_to_le16(PRB_PROT_READ);
	}
	fis.device = ATA_LBA;
	fis.device |= (block >> 24) & 0xf;
	fis.lba_high = (block >> 16) & 0xff;
	fis.lba_mid = (block >> 8) & 0xff;
	fis.lba_low = block & 0xff;
	fis.sector_count = blkcnt & 0xff;
	
	ret = ahci_exec_cmd(dev_no, )
	...
	/* Read from ahci */
	if (get_ahci_device_data(dev_no, (u8 *) & fis, sizeof(fis),
				 buffer, blkcnt * ATA_SECT_SIZE);
		debug("ahci: SCSI READ10 command failure.\n");
		return -1;
	}
