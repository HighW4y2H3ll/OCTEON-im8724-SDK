...
int usb_lowlevel_init(int index, enum usb_init_type init, void **controller)
{
	...
	if (ehci_readl(&ctrl->hccr->cr_hccparams) & 1)
		ehci_writel(&ctrl->hcor->or_ctrldssegment,
			    (addr >> 16) >> 16);
...
#endif
	for (i = 0; i < 1024; i++) {
		ctrl->periodic_list[i] = cpu_to_hc32((unsigned long)addr
						| QH_LINK_TYPE_QH);
	}
...
#ifdef CONFIG_OCTEON
        addr = virt_to_phys(ctrl->periodic_list);
#else
        addr = (uint32_t)ctrl->periodic_list;
#endif

	ehci_writel(&ctrl->hcor->or_periodiclistbase, addr);
		 ...
		 int elementsize, void *buffer, int interval)
{
	...
	for (i = 0; i < queuesize; i++) {
...
#else
		addr = (unsigned long)(qh+1);
#endif
		qh->qh_link = cpu_to_hc32((unsigned long)addr | QH_LINK_TYPE_QH);
		if (i == queuesize - 1)
		...
		addr = (uint32_t)td;
#endif
		qh->qh_overlay.qt_next = cpu_to_hc32((uint32_t)addr);
		qh->qh_overlay.qt_altnext = cpu_to_hc32(QT_NEXT_TERMINATE);
		qh->qh_endpt1 =
...
#else
		addr = (unsigned long)buffer + i * elementsize;
#endif
		td->qt_buffer[0] = cpu_to_hc32((unsigned long)addr);
		td->qt_buffer[1] =
			cpu_to_hc32((addr + 0x1000) & ~0xfff);
		td->qt_buffer[2] =
			cpu_to_hc32((addr + 0x2000) & ~0xfff);
		td->qt_buffer[3] =
			cpu_to_hc32((addr + 0x3000) & ~0xfff);
		td->qt_buffer[4] =
			cpu_to_hc32((addr + 0x4000) & ~0xfff);
		td->qt_buffer_hi[0] =
			cpu_to_hc32((addr >> 16) >> 16);
...
void *poll_int_queue(struct usb_device *dev,
