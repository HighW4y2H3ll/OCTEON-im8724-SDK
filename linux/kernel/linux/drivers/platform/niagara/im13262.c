...
IM13262_SFP_ATTR(0, 0);
IM13262_SFP_ATTR(0, 1);
IM13262_SFP_ATTR(1, 0);
IM13262_SFP_ATTR(1, 1);

#define IM13262_DIAG_ATTR(m, n, k)						\
...
IM13262_DIAG_ATTR(0, 0, PIM0_I2C_SFP_CLIENTS_START);
IM13262_DIAG_ATTR(0, 1, (PIM0_I2C_SFP_CLIENTS_START+1));
IM13262_DIAG_ATTR(1, 0, PIM1_I2C_SFP_CLIENTS_START);
IM13262_DIAG_ATTR(1, 1, (PIM1_I2C_SFP_CLIENTS_START+1));

#define IM13262_LED_ATTR(_pim, _name, _reg, _shift)			\
...
IM13262_LED_ATTR(0, system, 0x8016, 0);
IM13262_LED_ATTR(1, system, 0x8116, 0);

#define IM13262_CFG_LED_ATTR(_pim, _reg)				\
...
IM13262_CFG_LED_ATTR(0, 0x8048);
IM13262_CFG_LED_ATTR(1, 0x8148);

#define IM13262_SFP_LED_ATTR(_pim, _offset, _name, _reg, _shift)			\
...
IM13262_SFP_LED_ATTR(0, 2, sfp0, 0x804c, 0);
IM13262_SFP_LED_ATTR(0, 1, sfp1, 0x804c, 4);

IM13262_SFP_LED_ATTR(1, 2, sfp0, 0x814c, 0);
IM13262_SFP_LED_ATTR(1, 1, sfp1, 0x814c, 4);

// TODO KG check bits in registers
#define IM13262_MISC_ATTR(m)                                      \
...
struct im13262 {
	...
	struct attribute_group *sfp_group[2];
...
static int im13262_i2c_init(int id);

int im13262_init(int id, struct kobject *parent)
{
	int i, err;
	char buf[32];

	im13262[id].pdev = platform_device_register_simple("im13262", id, NULL, 0);
	if (!im13262[id].pdev)
		return -ENOMEM;

	snprintf(buf, sizeof(buf), "pim%d", id);
	im13262[id].kobj = kobject_create_and_add(buf, parent);
	if (!im13262[id].kobj) {
		err = -ENODEV;
		goto out_pdev;
	}
	...
	im13262[id].kobj_led = kobject_create_and_add("led", im13262[id].kobj);
	if (!im13262[id].kobj_led) {
		err = -ENODEV;
		goto out_kobj;
	}

	err = sysfs_create_file(im13262[id].kobj, &pim_type.attr);
	if (err)
		goto out_kobj_led;

	for (i = 0; i < ARRAY_SIZE(im13262[id].sfp_group); i++) {
		err = sysfs_create_group(im13262[id].kobj, im13262[id].sfp_group[i]);
		if (err)
			goto out_sfp_group;
	}

	for (i = 0; i < ARRAY_SIZE(im13262[id].led_group); i++) {
		err = sysfs_create_group(im13262[id].kobj_led, im13262[id].led_group[i]);
		if (err)
			goto out_led_group;
	}

	err = sysfs_create_group(im13262[id].kobj, im13262[id].misc_group);
	if (err)
		goto out_misc_group;

	err = im13262_i2c_init(id);
	if (err)
		goto out_led_group;
...
out_misc_group:
	sysfs_remove_group(im13262[id].kobj, im13262[id].misc_group);

out_led_group:
	for (i--; i >= 0; i--)
		sysfs_remove_group(im13262[id].kobj_led, im13262[id].led_group[i]);
	i = ARRAY_SIZE(im13262[id].sfp_group);
out_sfp_group:
	for (i--; i >= 0; i--)
		sysfs_remove_group(im13262[id].kobj, im13262[id].sfp_group[i]);
	sysfs_remove_file(im13262[id].kobj, &pim_type.attr);
out_kobj_led:
	kobject_put(im13262[id].kobj_led);
out_kobj:
	kobject_put(im13262[id].kobj);
out_pdev:
	platform_device_del(im13262[id].pdev);
	return err;
}
// TODO KG check temp sensor
...
static int im13262_i2c_init(int id)
{
	...
	im13262[id].kobj_i2c = kobject_create_and_add("diag", im13262[id].kobj);
	if (!im13262[id].kobj_i2c) {
		return -ENODEV;
	}

	for (indx = 0; indx < ARRAY_SIZE(im13262[id].diag_group); indx++) {
		err = sysfs_create_group(im13262[id].kobj_i2c, im13262[id].diag_group[indx]);
		if (err)
			return -ENODEV;
	}
	pr_info("pim%d: im13262 probed\n", id);
	return 0;
}

int im13262_i2c_deinit(int id)
{
	int shift = id ?PIM1_I2C_SFP_CLIENTS_START:
			PIM0_I2C_SFP_CLIENTS_START;
	int indx;
	...
	if (im13262[id].kobj_i2c) {
		for (indx = 0; indx < ARRAY_SIZE(im13262[id].diag_group); indx++)
			sysfs_remove_group(im13262[id].kobj_i2c, im13262[id].diag_group[indx]);
	}
